...
- **DOM structure change (f1fantasytools.com):** The site switched from fixed column indices to `data-column-id` attributes on `<th>` elements. Extraction functions now build a dynamic `colMap` from these attributes instead of hardcoding cell indices like `cells[15]`. This makes extraction resilient to column reordering. Table identification uses unique column IDs (e.g., `R_dotdPoints` for drivers, `R_fastestPitStopPoints`/`Q_teamworkPoints` for constructors) with a fallback to the `asset` column header text (`DR`/`CR`).
- **Template literal `\\n` escaping:** Inside backtick-wrapped function strings used with `page.evaluate()`, literal backslash-n (`\\n`) must be double-escaped so it survives string construction and reaches the browser context as `\n` for `split()`.
